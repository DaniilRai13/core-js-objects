...
function mergeObjects(objects) {
  ...
  objects.forEach((obj) => {
    Object.entries(obj).forEach(([key, val]) => {
      if (!res.length && Object.prototype.hasOwnProperty.call(res, key)) {
        const v = val + res[key];
        res = { ...res, [key]: v };
      } else {
        res = { ...res, [key]: val };
...
function makeWord(lettersObject) {
  ...
  Object.entries(lettersObject).forEach(([key, arrOfIndex]) => {
    arrOfIndex.forEach((idx) => {
      res[idx] = key;
    });
  });
...
function group(array, keySelector, valueSelector) {
  return array.reduce((acc, item) => {
    const getValue = valueSelector(item);
    const getKey = keySelector(item);
    acc.set(getKey, (acc.get(getKey) || []).concat(getValue));
    return acc;
  }, new Map());
}
